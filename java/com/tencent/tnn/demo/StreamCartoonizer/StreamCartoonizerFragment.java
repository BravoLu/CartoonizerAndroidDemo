package com.tencent.tnn.demo.StreamCartoonizer;
...
public class StreamCartoonizerFragment extends BaseFragment {

    private final static String TAG = StreamCartoonizerFragment.class.getSimpleName();
    ...
    private ImageCartoonization mCartoonizer = new ImageCartoonization();
    private FpsCounter mFpsCounter = new FpsCounter();
    private boolean mIsCartoonizing = false;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        NpuEnable = mCartoonizer.checkNpu(modelPath);
    }
...
//                popTip("can't find camera","");
                ...
                if (r == 0) {
                    ...
                    int ret = mCartoonizer.init(modelPath, mCameraWidth, mCameraHeight, device);
                    if (ret == 0) {
                        mIsCartoonizing = true;
                    } else {
                        mIsCartoonizing = false;
                        Log.e(TAG, "Cartoonization init failed " + ret);
                    }
                ...
                mOpenedCamera.setPreviewCallback(new Camera.PreviewCallback() {
                    ...
                    public void onPreviewFrame(byte[] data, Camera camera) {
                        if (mIsCartoonizing) {
                            ...
                            // objectInfoList = mCartoonizer.cartoonizeFromStream(data, mCameraParameters.getPreviewSize().width, mCameraParameters.getPreviewSize().height, mDrawView.getWidth(), mDrawView.getHeight(), mRotate);
                            imageInfoList = mCartoonizer.cartoonizeFromStream(data, mCameraParameters.getPreviewSize().width, mCameraParameters.getPreviewSize().height, mRotate);
                            if (mIsCountFps) {
                            ...
                            Log.i(TAG, "detect from stream ret " + imageInfoList);
                            // mDrawView.addObjectRect(objectInfoList,  ObjectDetector.label_list);
                            mDrawView.addImageInfo(imageInfoList[0]);
                        }
                        else {
                            Log.i(TAG,"No object");
                        }
